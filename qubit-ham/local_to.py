...
def bath_gibbs_states(beta_a=1.0, beta_b=2.0, bath_dim=4, omega_a=1.0, omega_b=1.0):
    """Gibbs states for both baths."""
    Ha = sho_ham(bath_dim, omega_a)
    Hb = sho_ham(bath_dim, omega_b)
    return gibbs_state(Ha, beta_a), gibbs_state(Hb, beta_b)

def sys_gibbs_states(beta_a=1.0, beta_b=2.0, w1=1.0, w2=2.0):
    """Gibbs states for both qubits via their individual bath temperatures."""
    H1 = qubit_ham(w1)
    H2 = qubit_ham(w2)
    return gibbs_state(H1, beta_a), gibbs_state(H2, beta_b)
...
def sanity_check_gibbs(beta_a=1.0, beta_b=2.0, w1=1.0, w2=2.0, omega=1.0, bath_dim=4, tol=1e-8):
    """
    ...
    Hb1 = sho_ham(bath_dim, omega)
    Hb2 = sho_ham(bath_dim, omega)
